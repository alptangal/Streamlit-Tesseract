...
async def on_ready():
    ...
    try:
      ...
      channel=guild.get_channel(1236962672323330101)
      await channel.send(rs)
      if rs:
        INFO=rs
...
async def getTransMb(guild): 
  ...
  if INFO:
    try:
      ...
      if rs:
        for item in rs:
          iss=item['creditAmount'] if item['creditAmount']!='0' else item['debitAmount']
          print(item['description'],iss,[f'{cur:,}' for cur in [int(iss)]][0])
          applied_tags=[]
